...
#define STATUS_FOUND_TRANSLATE 2

#define YUBIKEY_TOKEN_LENGTH 44
#define YUBIKEY_TOKEN_WITH_KEYMAP_LENGTH 60
#define YUBIKEY_ID_LENGTH 12
#define YUBIKEY_KEYMAP_LENGTH 16
#define YUBIKEY_DEFAULT_KEYMAP "cbdefghijklnrtuv"
...
#define DEFAULT_VALIDATION_URL "http://api.yubico.com/wsapi/2.0/verify"
...
static char *translateString(const char *string, const char *keymap, int len, request_rec *r) {
	int i = 0;
	int j = 0;

	char *newString = apr_pstrndup(r->pool, string, (apr_size_t) len);

	for (i = 0; i < len; i++) {
		for (j = 0; j < strlen(YUBIKEY_DEFAULT_KEYMAP); j++) {
			if (string[i] == keymap[j]) {
				newString[i] = YUBIKEY_DEFAULT_KEYMAP[j];
				break;
			}
		}
	}
		       ...
		       request_rec *r)
{
    ...
    char *yubiKeyId = NULL;
    char *yubiKeyId_trans = NULL;
    char *userPassword = NULL;
    char *userPassword_trans = NULL;
    char *keymap = NULL;
     ...
     */
     ...
     if (keymap) {
	     yubiKeyId_trans = translateString(yubiKeyId, keymap, YUBIKEY_ID_LENGTH, r);
	     ap_log_rerror(APLOG_MARK, APLOG_ERR|APLOG_DEBUG, 0, r, LOG_PREFIX "The translated calculated YubiKey ID is: %s", yubiKeyId_trans);
     }
	 ...
	 * want.
	 */
        if (!strncmp(yubiKeyId, w, 12) || (yubiKeyId_trans && !strncmp(yubiKeyId_trans, w, 12))) {
	   ...
	   */
	  status = apr_password_validate(unPw?unPw:user, realName);
                                    ...
                                    const char *password)
{
	...
	if (cfg->serverKey) {
		ykclient_set_client_hex(ykc, cfg->keyId, cfg->serverKey);
	} else {
		ykclient_set_verify_signature(ykc, 0);
		ykclient_set_client(ykc, cfg->keyId, 0, NULL);
	}
...
static const command_rec authn_yubikey_cmds[] = {
    ...
    AP_INIT_TAKE1("AuthYubiKeyValidationUrl", ap_set_string_slot,
    ...
    AP_INIT_TAKE1("AuthYubiKeyServerKeyId", ap_set_int_slot,
                  (void*) APR_OFFSETOF(yubiauth_dir_cfg, keyId),
                  ACCESS_CONF, "The Server Key Id"),
    AP_INIT_TAKE1("AuthYubiKeyServerKey", ap_set_string_slot,
                  (void*) APR_OFFSETOF(yubiauth_dir_cfg, serverKey),
                  ACCESS_CONF, "The Server Key"),
    {NULL}
};
...
static void *merge_yubiauth_dir_cfg(apr_pool_t *pool, void *BASE, void *ADD)
{
  ...
  dir->keyId = (add->keyId == NULL)? base->keyId : add->keyId;
  dir->serverKey = (add->serverKey == NULL)? base->serverKey : add->serverKey;

  dir->validationUrl = (add->validationUrl == NULL)? base->validationUrl: add->validationUrl;
  ...
  if (dir->validationUrl == NULL) dir->validationUrl = DEFAULT_VALIDATION_URL;
  
  if (dir->keyId == 0) {
    dir->keyId = DEFAULT_KEY_ID;
  }
